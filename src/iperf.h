...
struct stream_burst_metrics {
  ...
  // Next
  SLIST_ENTRY(stream_burst_metrics) sender_burst_metrics;
  SLIST_ENTRY(stream_burst_metrics) receiver_burst_metrics;
};
...
struct iperf_stream {
  ...
  SLIST_HEAD(sblisthead, stream_burst_metrics) sender_burst_metrics;
  SLIST_HEAD(rblisthead, stream_burst_metrics) receiver_burst_metrics;
...
struct iperf_test {
  ...
  iperf_size_t
      bitrate_limit_last_interval_index; /* Index of the last interval traffic
                                            inserted into the cyclic array */
...
/* UDP "connect" message and reply (textual value for Wireshark, etc.
 * readability - legacy was numeric) */
...
/* In Reverse mode, maximum number of packets to wait for "accept" response - to
