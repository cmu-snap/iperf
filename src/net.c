...
int timeout_connect(int s, const struct sockaddr *name, socklen_t namelen,
  ...
  if (timeout != -1 && fcntl(s, F_SETFL, flags) == -1) ret = -1;
...
int create_socket(int domain, int proto, const char *local,
  ...
  if (local) {
    ...
    if ((gerror = getaddrinfo(local, NULL, &hints, &local_res)) != 0) return -1;
  }
  ...
  if ((gerror = getaddrinfo(server, portstr, &hints, &server_res)) != 0) {
    if (local) freeaddrinfo(local_res);
    return -1;
  }
...
int netannounce(int domain, int proto, const char *local, const char *bind_dev,
  ...
  if ((gerror = getaddrinfo(local, portstr, &hints, &res)) != 0) return -1;
...
int NreadCompute(int fd, char *buf, size_t count, int prot,
  ...
  while (nleft > 0) {
    ...
    if (r < 0) {
      switch (errno) {
        ...
        case EWOULDBLOCK:
#endif
          if (count == nleft) return NET_SOFTERROR;
          return count - nleft;
