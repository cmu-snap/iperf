...
retry:
  ...
  if (!test->json_output) {
    if (test->server_last_run_rc != 2) test->server_test_number += 1;
    if (test->debug || test->server_last_run_rc != 2) {
      ...
      iperf_printf(
          test,
          "-----------------------------------------------------------\n");
      if (test->forceflush) iflush(test);
    }
  }
...
int iperf_accept(struct iperf_test *test) {
  ...
  if (test->ctrl_sck == -1) {
    ...
    if (iperf_set_send_state(test, PARAM_EXCHANGE) != 0) return -1;
    if (iperf_exchange_parameters(test) < 0) return -1;
    if (test->server_affinity != -1)
      if (iperf_setaffinity(test, test->server_affinity) != 0) return -1;
    ...
    if (Nwrite(s, (char *)&rbuf, sizeof(rbuf), Ptcp) < 0) {
      if (test->debug)
        printf(
            "failed to send ACCESS_DENIED to an unsolicited connection request "
            "during active test\n");
    } else {
      if (test->debug)
        printf(
            "successfully sent ACCESS_DENIED to an unsolicited connection "
...
int iperf_handle_message_server(struct iperf_test *test) {
  ...
  switch (test->state) {
    ...
    case TEST_END:
      ...
      if (iperf_set_send_state(test, EXCHANGE_RESULTS) != 0) return -1;
      if (iperf_exchange_results(test) < 0) return -1;
      if (iperf_set_send_state(test, DISPLAY_RESULTS) != 0) return -1;
...
static void server_stats_timer_proc(TimerClientData client_data,
  ...
  if (test->done) return;
  if (test->stats_callback) test->stats_callback(test);
}
...
static void server_reporter_timer_proc(TimerClientData client_data,
  ...
  if (test->done) return;
  if (test->reporter_callback) test->reporter_callback(test);
}
...
static void server_omit_timer_proc(TimerClientData client_data,
  ...
  /* Reset the timers. */
  if (test->stats_timer != NULL) tmr_reset(nowP, test->stats_timer);
  if (test->reporter_timer != NULL) tmr_reset(nowP, test->reporter_timer);
}
...
int iperf_run_server(struct iperf_test *test) {
  ...
  if (test->logfile)
    if (iperf_open_logfile(test) < 0) return -2;
      ...
      if (iperf_time_diff(&now, &last_receive_time, &diff_time) == 0) {
        ...
        if (test->state == IPERF_START) {
          if (test->settings->idle_timeout > 0 &&
            ...
            if (test->debug)
              printf(
                  "Server restart (#%d) in idle state as no connection request "
                  "was received for %d sec\n",
            ...
            if (iperf_get_test_one_off(test)) {
              if (test->debug)
                printf(
                    "No connection request was received for %d sec in one-off "
                    "mode; exiting.\n",
                    test->settings->idle_timeout);
              exit(0);
            }
        ...
        if (FD_ISSET(test->prot_listener, &read_set)) {
          ...
          if (!is_closed(s)) {
            ...
            if (flag != -1) {
              ...
              if (test->on_new_stream) test->on_new_stream(sp);
  ...
  if (test->json_output) {
    if (iperf_json_finish(test) < 0) return -1;
  }
  ...
  if (test->server_affinity != -1)
    if (iperf_clearaffinity(test) != 0) return -1;
