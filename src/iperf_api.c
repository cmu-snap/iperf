...
int iperf_get_test_bind_port(struct iperf_test *ipt) { return ipt->bind_port; }
...
int iperf_get_test_mss(struct iperf_test *ipt) { return ipt->settings->mss; }

int iperf_get_mapped_v4(struct iperf_test *ipt) { return ipt->mapped_v4; }
...
struct protocol *get_protocol(struct iperf_test *test, int prot_id) {
  ...
  SLIST_FOREACH(prot, &test->protocols, protocols) {
    if (prot->id == prot_id) break;
  }

  if (prot == NULL) i_errno = IEPROTOCOL;
  ...
  if (test->json_output) {
    cJSON_AddItemToObject(
        ...
        iperf_json_printf(
            ...
            (int64_t)test->bidirectional, (uint64_t)test->settings->fqrate));
  } else {
    if (test->verbose) {
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv) {
    ...
    switch (flag) {
      ...
      case 'd':
        ...
        if (optarg) {
          ...
          if (test->debug_level < 0) test->debug_level = DEBUG_LEVEL_MAX;
        }
        break;
      case 'I':
  ...
  // File cannot be transferred using UDP because of the UDP packets header
  // (packet number, etc.)
  ...
  */
  if (test->settings->bytes == 0 && test->settings->blocks == 0 &&
      !duration_flag && test->diskfile_name != (char *)0 && test->role == 'c') {
    struct stat st;
...
void iperf_check_throttle(struct iperf_stream *sp, struct iperf_time *nowP) {
  ...
  if (sp->test->done || sp->test->settings->rate == 0) return;
...
int iperf_send_mt(struct iperf_stream *sp) {
  ...
  for (; multisend > 0; --multisend) {
    if (no_throttle_check) iperf_time_now(&now);
    streams_active = 0;
    {
...
int iperf_init_test(struct iperf_test *test) {
  ...
  if (test->protocol->init) {
    if (test->protocol->init(test) < 0) return -1;
  }
  ...
  if (test->on_test_start) test->on_test_start(test);
...
int iperf_exchange_parameters(struct iperf_test *test) {
  ...
  if (test->role == 'c') {
    if (send_parameters(test) < 0) return -1;

  } else {
    if (get_parameters(test) < 0) return -1;

#if defined(HAVE_SSL)
    if (test_is_authorized(test) < 0) {
      if (iperf_set_send_state(test, SERVER_ERROR) != 0) return -1;
      i_errno = IEAUTHTEST;
      err = htonl(i_errno);
    ...
    if (iperf_set_send_state(test, CREATE_STREAMS) != 0) return -1;
  }
...
int iperf_exchange_results(struct iperf_test *test) {
  if (test->role == 'c') {
    ...
    if (send_results(test) < 0) return -1;
    /* Get server results. */
    if (get_results(test) < 0) return -1;
  } else {
    /* Get client results. */
    if (get_results(test) < 0) return -1;
...
static int send_parameters(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    if (test->no_delay) cJSON_AddTrueToObject(j, "nodelay");
    cJSON_AddNumberToObject(j, "parallel", test->num_streams);
    if (test->reverse) cJSON_AddTrueToObject(j, "reverse");
    if (test->bidirectional) cJSON_AddTrueToObject(j, "bidirectional");
    ...
    if (test->title) cJSON_AddStringToObject(j, "title", test->title);
    if (test->extra_data)
    ...
    if (test->zerocopy) cJSON_AddNumberToObject(j, "zerocopy", test->zerocopy);
#if defined(HAVE_DONT_FRAGMENT)
...
static int get_parameters(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    if ((j_p = cJSON_GetObjectItem(j, "tcp")) != NULL) set_protocol(test, Ptcp);
    if ((j_p = cJSON_GetObjectItem(j, "udp")) != NULL) set_protocol(test, Pudp);
    ...
    if ((j_p = cJSON_GetObjectItem(j, "nodelay")) != NULL) test->no_delay = 1;
            ...
            if (j_id == NULL || j_bytes == NULL || j_retransmits == NULL ||
              ...
              r = -1;
                       ...
                       (j_omitted_errors != NULL &&
                        j_omitted_packets == NULL)) {
              /* For backward compatibility allow to not receive "omitted"
               * statistcs */
              i_errno = IERECVRESULTS;
              r = -1;
            } else {
                ...
                if (sp->sender) {
                  ...
                  if (j_omitted_packets != NULL) {
                    ...
                    if (sp->peer_omitted_packet_count > 0) {
                      /* -1 indicates unknown error count since it includes the
                       * omitted count */
...
static int JSON_write(int fd, cJSON *json) {
  ...
  if (str == NULL)
    ...
    if (Nwrite(fd, (char *)&nsize, sizeof(nsize), Ptcp) < 0)
      ...
      if (Nwrite(fd, str, hsize, Ptcp) < 0) r = -1;
    }
    cJSON_free(str);
  }
  return r;
...
void iperf_free_test(struct iperf_test *test) {
  ...
  if (test->server_hostname) free(test->server_hostname);
  if (test->tmp_template) free(test->tmp_template);
  if (test->bind_address) free(test->bind_address);
  if (test->bind_dev) free(test->bind_dev);
  ...
  if (test->server_rsa_private_key) EVP_PKEY_free(test->server_rsa_private_key);
  ...
  if (test->settings) free(test->settings);
  if (test->title) free(test->title);
  if (test->extra_data) free(test->extra_data);
  if (test->congestion) free(test->congestion);
  if (test->congestion_used) free(test->congestion_used);
  if (test->remote_congestion_used) free(test->remote_congestion_used);
  if (test->timestamp_format) free(test->timestamp_format);
  if (test->omit_timer != NULL) tmr_cancel(test->omit_timer);
  if (test->timer != NULL) tmr_cancel(test->timer);
  if (test->stats_timer != NULL) tmr_cancel(test->stats_timer);
  if (test->reporter_timer != NULL) tmr_cancel(test->reporter_timer);
...
static void iperf_print_intermediate(struct iperf_test *test) {
  ...
  for (current_mode = lower_mode; current_mode <= upper_mode; ++current_mode) {
    ...
    if (test->num_streams > 1 || test->json_output) {
      /*
       * With BIDIR give a different JSON object name to the one sent/receive
       * sums. The different name is given to the data sent from the server,
       * which is the "reverse" channel.  This makes sure that the name reported
       * on the server and client are compatible, and the names are the same as
       * with non-bidir, except for when reverse is used.
       */
      sum_name = "sum";
      if (test->mode == BIDIRECTIONAL) {
      ...
      SLIST_FOREACH(sp, &test->streams, streams) {
        if (sp->sender == stream_must_be_sender) {
          if (test->json_output) {
            ...
            if (json_summary_stream == NULL) return;
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
             ...
             * count.
             */
            int64_t packet_count = sender_packet_count ? sender_packet_count
            ...
            if (test->json_output) {
              ...
              if (test->role == 's' && !sp->sender) {
                ...
                iperf_printf(
                    ...
                    sender_time, ubuf, nbuf, 0.0, (int64_t)0,
                    (sender_packet_count - sender_omitted_packet_count),
            ...
            if (!test->json_output) {
              ...
              if (test->role == 's' && sp->sender) {
                ...
                if (sp->omitted_cnt_error > -1) {
                  ...
                  iperf_printf(
                      test, report_bw_udp_format_no_omitted_error, sp->socket,
                      mbuf, start_time, receiver_time, ubuf, nbuf,
                      sp->jitter * 1000.0,
                      (receiver_packet_count - receiver_omitted_packet_count),
                      report_receiver);
                }
        ...
        if (test->json_output) {
          ...
          cJSON_AddItemToObject(
              ...
              iperf_json_printf(
                  ...
                  (int64_t)total_sent, (double)total_sent * 8 / sender_time,
                  (double)0.0, (int64_t)0, (int64_t)sender_total_packets,
                  (double)0.0, 1));
          cJSON_AddItemToObject(
              test->json_end, sum_received_name,
              iperf_json_printf(
                  ...
                  (double)avg_jitter * 1000.0, (int64_t)lost_packets,
                  (int64_t)receiver_total_packets, (double)lost_percent, 0));
        } else {
          /*
          ...
          if (!(test->role == 's' && !stream_must_be_sender)) {
            ...
            iperf_printf(test, report_sum_bw_udp_format, mbuf, start_time,
                         sender_time, ubuf, nbuf, 0.0, (int64_t)0,
                         sender_total_packets, 0.0, report_sender);
          }
          if (!(test->role == 's' && stream_must_be_sender)) {
            unit_snprintf(ubuf, UNIT_LEN, (double)total_received, 'A');
...
static void print_interval_results(struct iperf_test *test,
  ...
  if (test->logfile || test->forceflush) iflush(test);
}
...
void iperf_free_stream(struct iperf_stream *sp) {
  ...
  close(sp->buffer_fd);
  if (sp->diskfile_fd >= 0) close(sp->diskfile_fd);
...
static int diskfile_send(struct iperf_stream *sp) {
  ...
  if (sp->diskfile_left < sp->test->settings->blksize && !sp->test->done) {
    ...
    if (buffer_left != sp->test->settings->blksize) {
      if (sp->test->debug) printf("possible eof\n");
  ...
  if (sp->test->done || buffer_left == 0) {
    if (sp->test->debug) printf("already done\n");
    sp->test->done = 1;
    return 0;
  }
...
int iperf_json_finish(struct iperf_test *test) {
  ...
  test->json_start = test->json_connected = test->json_intervals =
      test->json_server_output = test->json_end = NULL;
  return 0;
}
...
int iperf_clearaffinity(struct iperf_test *test) {
  ...
  CPU_ZERO(&cpu_set);
  for (i = 0; i < CPU_SETSIZE; ++i) CPU_SET(i, &cpu_set);
  if (sched_setaffinity(0, sizeof(cpu_set_t), &cpu_set) != 0) {
    i_errno = IEAFFINITY;
    return -1;
  }
