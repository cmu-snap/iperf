...
void *iperf_client_worker_run(void *s) {
  ...
  while (!(test->done) && !(sp->done)) {
    ...
    if (sp->sender) {
      ...
      // Error: We only support the client being the sender, but this is the client and
      // we have been asked to receive!
      goto cleanup_and_fail;
    }
...
int iperf_connect(struct iperf_test *test) {
  ...
  if (test->ctrl_sck < 0)
    ...
    test->ctrl_sck =
        netdial(test->settings->domain, Ptcp, test->bind_address,
                test->bind_dev, 0, test->server_hostname, test->server_port,
...
int iperf_client_end(struct iperf_test *test) {
  ...
  if (test->state > 0) {
    if (iperf_set_send_state(test, IPERF_DONE) != 0) return -1;
  }

  /* Close control socket */
  if (test->ctrl_sck >= 0) close(test->ctrl_sck);
...
int iperf_run_client(struct iperf_test *test) {
  ...
  if (test->logfile)
    if (iperf_open_logfile(test) < 0) return -1;

  if (test->affinity != -1)
    if (iperf_setaffinity(test, test->affinity) != 0) return -1;
  ...
  if (iperf_connect(test) < 0) goto cleanup_and_fail;
  ...
  while (test->state != IPERF_DONE) {
    ...
    if (test->state == TEST_RUNNING) {
      ...
      if ((!test->omitting) &&
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
          if (sp->sender) {
            ...
            if (rc != 0 && rc != ESRCH) {
              ...
              errno = rc;
              iperf_err(test, "sender cancel in pthread_cancel - %s",
            ...
            if (rc != 0 && rc != ESRCH) {
              ...
              errno = rc;
              iperf_err(test, "sender cancel in pthread_join - %s",
                        iperf_strerror(i_errno));
              goto cleanup_and_fail;
            }
            if (test->debug_level >= DEBUG_LEVEL_INFO) {
        ...
        if (iperf_set_send_state(test, TEST_END) != 0) goto cleanup_and_fail;
      }
    }
  }
